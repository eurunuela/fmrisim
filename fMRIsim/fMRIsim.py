...
def fMRIsim_workflow(out_file='simulation', out_dir='data', length='mix',
                     nxyz=[1, 1, 1], duration=400, tr=2, te=None, nevents=2,
                     gap=10, maxlength=None, minlength=None, group=None, afni=False,
                     hrfmodel='SPMG1', hrfpath=None, tesla=3, no_noise=False,
                     snr=10, motpath=None, npy=False, history=''):
    ...
    for te_idx in range(len(sim.te)):
        ...
        temp_sim = sim.simulation[te_idx * sim.nscans:(te_idx + 1) *
                                  sim.nscans, :].copy()
        vol.export_volume(
            temp_sim, dims, out_dir,
            '{}_data_E0{}'.format(out_file, str(te_idx + 1)), history)

        # Each of the TE BOLD data
        temp_bold = sim.bold[te_idx * sim.nscans:(te_idx + 1) *
                             sim.nscans, :].copy()
        vol.export_volume(
            temp_bold, dims, out_dir,
            '{}_bold_E0{}'.format(out_file, str(te_idx + 1)), history)
        ...
        vol.export_volume(
            temp_noise, dims, out_dir,
            '{}_noise_E0{}'.format(out_file, str(te_idx + 1)), history)

    # Non TE dependent data
    vol.export_volume(sim.r2, dims, out_dir, out_file + '_r2', history)
    vol.export_volume(sim.innovation, dims, out_dir,
                      out_file + '_innovation', history)

    if npy:
        np.save('{}/{}_data'.format(out_dir, out_file),
                sim.simulation)  # Data
        np.save('{}/{}_bold'.format(out_dir, out_file),
                sim.bold)  # BOLD
        np.save('{}/{}_noise'.format(out_dir, out_file),
                sim.noise)  # Noise
        np.save('{}/{}_r2'.format(out_dir, out_file), sim.r2)  # R2
        np.save('{}/{}_innovation'.format(out_dir, out_file),
