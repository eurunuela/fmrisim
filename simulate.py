...
class fMRIsim:
    ...
    def __init__(self, event_length='mix', nvoxels=1, dur=400, TR=2, snr=10, nevents=1, 
            gap=5, TE=None, is_afni=True, lop_hrf='SPMG1', has_integrator=True, tesla=3,
            noise=True, max_length = None, min_length=None, group=1):
    ...
    def _add_noise(self):
        ...
        # SNR simulations based on Tryantafyllou NI,2005 and van der Zwaag NI,2009
        # Relative signal change (DeltaS/S) at 1x1x3
        # (1.5T with TE=50ms = 0.02, 3T with TE=35=0.03, 7T with TE=25ms = 0.06)
        # Consider mean of signal = 1, then sigma_noise = 1/SNR
        self.S = 1
        ...
        sigma_noise = self.S/self.snr
        sps0 = par_sps0[0]*(np.power(self.snr,par_sps0[1]))+par_sps0[2]
        sigma_thermalnoise = np.sqrt((np.power(sigma_noise,2))/((np.power(sps0,2))+1))
        ...
        s_cardiac = (np.power(power_harmonics, 0))*np.sin(2*np.pi*fcardiac*self.tr*np.linspace(0,self.nscans-1,self.nscans)+2*np.pi*np.random.rand(1)[0])
        s_resp = (np.power(power_harmonics, 0))*np.sin(2*np.pi*fresp*self.tr*np.linspace(0,self.nscans-1,self.nscans)+2*np.pi*np.random.rand(1)[0])
        for j in range(n_harmonics-1):
            s_cardiac = s_cardiac + (np.power(power_harmonics,(-j)))*np.sin(2*np.pi*(j+1)*fcardiac*self.tr*np.linspace(0,self.nscans-1,self.nscans)+2*np.pi*np.random.rand(1)[0])
            s_resp = s_resp + (np.power(power_harmonics,(-j)))*np.sin(2*np.pi*(j+1)*fresp*self.tr*np.linspace(0,self.nscans-1,self.nscans)+2*np.pi*np.random.rand(1)[0])
    ...
    def simulate(self):
        ...
        group_change_idxs = np.zeros((len(self.group) + 1,))
            ...
            if voxidx == group_change_idxs[group_label]:
                ...
                for eventidx in range(self.nevents):
                    if self.max_length and self.min_length:
                        temp_len = np.random.randint(self.min_length,self.max_length)
                        temp_pos = int(random.choice(idx_avail[:len(idx_avail)-temp_len]))
                        ...
                        if ev_len == 'short': # Event length [1,1%]
                            ...
                            temp_len = np.random.randint(np.ceil(0.05*self.nscans), self.max_length)
                            temp_pos = int(random.choice(idx_avail[:len(idx_avail)-temp_len]))
                            idx_pos = np.where(idx_avail == temp_pos)[0]
                            while (idx_avail[idx_pos+temp_len] - idx_avail[idx_pos]) > temp_len:
                                temp_len -= 1
                            self.r2[temp_pos:temp_pos+temp_len,voxidx] = 1
                            idx_sel = np.arange(idx_pos-2, idx_pos+temp_len+2)
                            np.delete(idx_avail, idx_sel)
        ...
        # Generate HRF matrix
        hrf_matrix = HRFMatrix(TR=self.tr, TE=self.te, nscans=self.nscans, r2only=True, 
            is_afni=self.is_afni, lop_hrf=self.lop_hrf, has_integrator=self.has_integrator)
        ...
        for te_idx in range(len(self.te)):
            temp_bold = self.bold[te_idx*self.nscans:(te_idx+1)*self.nscans,:].copy()
            temp_noise = self.noise[te_idx*self.nscans:(te_idx+1)*self.nscans,:].copy()
            temp_simulation = temp_bold + (temp_noise*np.max(temp_bold)/self.snr)
            self.simulation[te_idx*self.nscans:(te_idx+1)*self.nscans,:] = temp_simulation.copy()
